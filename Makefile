...
build:
	...
	go build -o artistcoin -ldflags="\
    	-X 'github.com/ArtistBlockchain/ArtistCoin/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/ArtistBlockchain/ArtistCoin/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/ArtistBlockchain/ArtistCoin/versioning.Branch=$(BRANCH)'\
		-X 'github.com/ArtistBlockchain/ArtistCoin/versioning.BuildTime=$(TIME)'" \
	main.go
...
.PHONY: test
test:
	go build -o artifacts/artistcoin .
